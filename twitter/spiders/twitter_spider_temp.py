...
class SearchSpider(scrapy.Spider):
    ...
    def start_requests(self):
        user_accounts_path = "./user_accounts_new.json"
        ...
        with open(user_accounts_path, "r+", encoding="utf-8") as fc:
            # 然后再慢慢遍历,看是否有新的账号进来
            local_user_accounts_list = json.load(fc).get("accounts")
            if local_user_accounts_list:
                for user in local_user_accounts_list:
